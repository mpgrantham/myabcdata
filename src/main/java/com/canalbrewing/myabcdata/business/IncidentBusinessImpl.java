...
public class IncidentBusinessImpl implements IncidentBusiness {
	...
	public void checkEntryPermission(int userId, int observedId) throws SQLException  {
		...
		if ( ! found.isPresent() ) {
			throw new MyAbcDataException(MyAbcDataException.UNAUTHORIZED, "User does not have Entry permission for Observed.");
		}
	}

	public void checkAdminPermission(int userId, int observedId) throws SQLException  {
		List<Observed> observedList = observedDao.getObservedByUser(userId);
		
		Optional<Observed> found = observedList.stream().filter(o -> o.getId() == observedId 
							&& o.isAdminRole()).findAny();
		if ( ! found.isPresent() ) {
			throw new MyAbcDataException(MyAbcDataException.UNAUTHORIZED, "User does not have Admin permission for Observed.");
		}
	}
