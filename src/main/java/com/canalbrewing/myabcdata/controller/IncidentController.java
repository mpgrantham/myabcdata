...
public class IncidentController {
	...
	public ResponseEntity<Incident> addIncident(@RequestHeader("Authorization") String sessionToken, 
						@RequestBody RequestIncident requestIncident) throws SQLException {
	...
	@DeleteMapping("/observed/{observedId}/incident/{incidentId}")
    ...
    public ResponseEntity<StatusMessage> handleException(Exception ex) {
        return new ResponseEntity<>(new StatusMessage(StatusMessage.ERROR, ex.getMessage()), HttpStatus.INTERNAL_SERVER_ERROR);
	}
	...
	public ResponseEntity<StatusMessage> handleMyAbcDataException(MyAbcDataException ex) {
		switch (ex.getExceptionCode()) {
			...
			case MyAbcDataException.UNAUTHORIZED:
				return new ResponseEntity<>(new StatusMessage(StatusMessage.ERROR, ex.getMessage()), HttpStatus.UNAUTHORIZED);
